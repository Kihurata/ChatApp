<script>
    ...
    function loadMessages() {
      ...
      db.get("chats").get(conversationKey).map().on((data, key) => {
        if (data && data.text) {
          // Avoid duplicate messages in our local array.
          if (!messages.find((msg) => msg.timestamp === data.timestamp)) {
            // @ts-ignore
            messages = [...messages, { key, ...data }].sort((a, b) => new Date(a.timestamp) - new Date(b.timestamp));
          }
        }
      });
    }
    ...
    async function sendMessage() {
      ...
      // Store the message in the shared "chats" node
      db.get("chats").get(conversationKey).set({
        text: newMessage,
        sender: currentUser,
